...
<body>
  ...
  <h2 class="hd2">Marvel Series III - 1992</h2>
  <div class="grid-container">
      <div  id = "search" >
          ...
          include "openconn.php";
          ini_set('display_errors', 1);
          $stickers = mysqli_query($conn,"SELECT COUNT(id) FROM UserSticker WHERE username LIKE '".$_SESSION['loggedIn']."' AND id IN (SELECT id FROM `Sticker` WHERE sticker_album_id = 3)");
          $row = mysqli_fetch_array($stickers);
          $stickers = $row[0];

          $totalStickers = mysqli_query($conn, "SELECT `number of stickers` FROM `Sticker Album` WHERE id LIKE 3");
      ...
      include "openconn.php";
      ini_set('display_errors', 1);
      $query = mysqli_query($conn,"SELECT * FROM UserSticker WHERE username LIKE '".$_SESSION['loggedIn']."' AND id IN (SELECT id FROM `Sticker` WHERE sticker_album_id = 3)");
      while ($row = mysqli_fetch_assoc($query)){
          $id = $row["id"];
          echo '<div class="grid-item" id="card"><img src = "uploads/3/'. $id. '.jpg" style="height:100%;width:100%;"></div>';
      }
      ?>
   </div>
